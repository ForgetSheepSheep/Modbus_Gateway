...
static void ModbusSetRelay1(uint16_t value);
static void ModbusSetRelay2(uint16_t value);
static void ModbusGetRelay1(uint16_t *value);
static void ModbusGetRelay2(uint16_t *value);
static void ModbusGetKey1(uint16_t *value);
static void ModbusGetKey2(uint16_t *value);
static void ModbusGetKey3(uint16_t *value);
static MbRegisterInstance_t g_MbRegisterInstanceTable[] =
{
    ...
    {.property = R | W, .address = 0x0004, .minValue = 0, .maxValue = 1, .ReadCb = ModbusGetRelay2,  .WriteCb = ModbusSetRelay2,},
    {.property = R,     .address = 0x0005, .minValue = 0, .maxValue = 0, .ReadCb = ModbusGetKey1,    .WriteCb = NULL,},
    {.property = R,     .address = 0x0006, .minValue = 0, .maxValue = 0, .ReadCb = ModbusGetKey2,    .WriteCb = NULL,},
    {.property = R,     .address = 0x0007, .minValue = 0, .maxValue = 0, .ReadCb = ModbusGetKey3,    .WriteCb = NULL,},
    {.property = R | W, .address = 0x0008, .minValue = APP_PARAM_MIN_SLAVE_ADDR, .maxValue = APP_PARAM_MAX_SLAVE_ADDR, .ReadCb = ModbusGetSlaveAddr, .WriteCb = ModbusSetSlaveAddr,},
};
...
static void ModbusSetRelay1(uint16_t value) { GPIODriverWrite(IO_RELAY1, (uint8_t)value); }
static void ModbusSetRelay2(uint16_t value) { GPIODriverWrite(IO_RELAY2, (uint8_t)value); }
static void ModbusGetRelay1(uint16_t *value) { *value = (uint16_t)GPIODriverRead(IO_RELAY1); }
static void ModbusGetRelay2(uint16_t *value) { *value = (uint16_t)GPIODriverRead(IO_RELAY2); }
static void ModbusGetKey1(uint16_t *value) { *value = (uint16_t)KeyDriverReadState(0); }
static void ModbusGetKey2(uint16_t *value) { *value = (uint16_t)KeyDriverReadState(1); }
static void ModbusGetKey3(uint16_t *value) { *value = (uint16_t)KeyDriverReadState(2); }
