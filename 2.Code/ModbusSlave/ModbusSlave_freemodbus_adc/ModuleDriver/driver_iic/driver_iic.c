...
int IICDriverWrite(uint16_t DevAddress, uint8_t *pData, uint16_t Size)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, DevAddress, pData, Size, 0xFFFF);
...
int IICDriverRead(uint16_t DevAddress, uint8_t *pData, uint16_t Size)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Receive(&hi2c1, DevAddress, pData, Size, 0xFFFF);
    if(status != HAL_OK) return -1;
